...
for obj in values:
    ...
    for pre in values[obj]:
        if (True): #(values["Magnet"][pre][0][-1] >= 500 or values["Magnetic Stick"][pre][0][-1] >= 2000 or values["Velcro"][pre][0][-1] >= 400 or values["Velcro Stick"][pre][0][-1] >= 3500)):# and ("random" not in pre) and ("gaussian" not in pre)):
            NUM_COLORS=2
    ...
    for pre in values[obj]:
        if ("gaussian" not in pre and "random" not in pre):#(values["Magnet"][pre][0][-1] >= 500 or values["Magnetic Stick"][pre][0][-1] >= 2000 or values["Velcro"][pre][0][-1] >= 400 or values["Velcro Stick"][pre][0][-1] >= 3500)):# and ("random" not in pre) and ("gaussian" not in pre)):
            if ("batch" in pre):
                i = 0
            else:
                i = 1
